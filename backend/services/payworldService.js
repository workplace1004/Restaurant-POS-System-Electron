...
function parseTerminalConnection(connectionString) {
  ...
  const ip = get('ip', 'ipAddress', 'ip_address', 'host', 'hostname');
  const portRaw = get('port', 'Port');
  const posId = get('posId', 'pos_id', 'terminalId', 'terminal_id');
  const currencyCode = get('currencyCode', 'currency', 'currency_code');
  ...
  return {
    ...
    timeoutMs: Number.parseInt(timeoutMsRaw || '60000', 10) || 60000,
  };
}
...
<vcs-device:abortCardEntryNotification xmlns:vcs-device="http://www.vibbek.com/device">
  ...
  generateFinancialTrxXml({ amountCents, syncNumber }) {
    const posIdElement = this.config.posId ? `  <posId>${this.config.posId}</posId>\n` : '';
    const currencyElement = this.config.currencyCode ? `    <currency>${this.config.currencyCode}</currency>\n` : '';
...
${currencyElement}    <transactionType>0</transactionType>
  ...
  async processPaymentAsync(sessionId) {
    ...
    try {
      const responseXml = await this.sendAndParse(xml, this.config.timeoutMs || 120000);
      const current = this.sessions.get(sessionId);
      if (!current) return;
